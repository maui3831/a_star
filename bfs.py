...
def breadth_first_search(start_pos, goal_pos):
    ...
    while queue:
        ...
        for neighbor_idx in current_node.neighbors:
            ...
            if (
                neighbor_node.position not in visited
                and neighbor_node.position not in in_queue_set
            ):
        ...
        log_message = [
            f"Processing node {current_node.index} at {current_node.position}. It is now explored."
        ]
        if list(queue):
            log_message.append("  Nodes currently in queue (g=cost):")
            for node_in_queue in queue:
                g_val = (
                    f"{node_in_queue.g:.0f}"
                    if node_in_queue.g != float("inf")
                    else "inf"
                )
                log_message.append(
            ...
            if current_node.position != goal_pos:
                log_message.append(
                    "  Goal not reached and queue is empty. No path exists."
                )
    ...
    if not any(
        "Path found!" in step[4][0]
        for step in steps
        if len(step[4]) > 0 and isinstance(step[4][0], str)
    ):
        steps.append(
            (
                None,
